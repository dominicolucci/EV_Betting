...
# Player name to Basketball Reference ID map (add more as needed)
player_name_to_id = {
...
def get_game_logs(player_id, season, stat_key):
    """
    Scrapes game logs for a given player and season.
    ...
    # Mapping from stat key to the cell index for that stat in the row
    stat_map = {
...
def calculate_z_score(mean1, std1, n1, mean2, std2, n2):
...
def analyze_players(player_names, stat_key, seasons):
    summaries = []
    significant_count = 0
    ...
    for player_name in player_names:
        ...
        if not player_id:
            summaries.append(f"Player '{player_name}' not found in player map.\n")
            continue
        ...
        for season in seasons:
            try:
                games = get_game_logs(player_id, season, stat_key)
                for g in games:
                    if g[stat_key] is not None:
                        if g["home"]:
                            home_stats.append(g[stat_key])
                        else:
                            away_stats.append(g[stat_key])
            except Exception as e:
                summaries.append(f"Error scraping {player_name} for season {season}: {e}\n")
        
        if home_stats and away_stats:
            mean_home, std_home = mean_std(home_stats)
            mean_away, std_away = mean_std(away_stats)
            n_home = len(home_stats)
            n_away = len(away_stats)
            z = calculate_z_score(mean_home, std_home, n_home, mean_away, std_away, n_away)

            summary = (
                f"--- {player_name.title()} Home vs Away {stat_key} Comparison ---\n"
                f"Home Games: {n_home}, Sum: {sum(home_stats):.1f}, Avg: {mean_home:.2f}, Std Dev: {std_home:.2f}\n"
                f"Away Games: {n_away}, Sum: {sum(away_stats):.1f}, Avg: {mean_away:.2f}, Std Dev: {std_away:.2f}\n"
                f"Z-score: {z:.2f}\n"
            )
            if abs(z) > 1.96:
                summary += ">> Statistically significant difference (p < 0.05)\n"
                significant_count += 1
            else:
                summary += ">> No statistically significant difference (p >= 0.05)\n"
            summaries.append(summary)
        else:
            summaries.append(f"Insufficient data to compute z-score for {player_name.title()}.\n")
    
    summaries.append(f"\n>>> {significant_count} out of {total_players} players showed statistically significant differences.\n")
    return "\n".join(summaries)

if __name__ == "__main__":
    num_players = int(input("How many players will you enter? "))
    player_names_input = input(f"Enter {num_players} player names (comma-separated, e.g., LeBron James, Luka Doncic, Joel Embiid): ").strip()
    player_names = [name.strip() for name in player_names_input.split(",") if name.strip()]
    if len(player_names) != num_players:
        print(f"Please enter exactly {num_players} names.")
        exit()
    
    stat_key = input("Enter stat (e.g., PTS, AST, etc.): ").strip().upper()
    
    season_input = input("Enter seasons (e.g., 2022,2023 or 2021-2025): ").strip()
    seasons = parse_seasons(season_input)
    
    # Perform analysis without printing individual game data
    output = analyze_players(player_names, stat_key, seasons)
    print("\n" + output)
